        ...
        for ( size_t j = 0; j < ima.getCount(); j++)
        {
            ...
            if (symbol.compare(materialName) == 0)
            {
                ...
                if (mat)
                {
                    ...
                    domMaterial *mat = daeSafeCast< domMaterial >(getElementFromURI( ima[j]->getTarget()));
                    saveMaterialToStateSetMetaData(mat, ss);
                    ...
                    if (copyTextureCoordinateSet(ss, cachedGeometry, clonedGeometry, ima[j], BUMP_MAP_UNIT , textureUnit)) ++textureUnit;
                    if (copyTextureCoordinateSet(ss, cachedGeometry, clonedGeometry, ima[j], IMAGE_BASE_LIGHT_MAP_UNIT , textureUnit)) ++textureUnit;
                    if (copyTextureCoordinateSet(ss, cachedGeometry, clonedGeometry, ima[j], ILLUMINATION_MAP_UNIT , textureUnit)) ++textureUnit;
                    if (copyTextureCoordinateSet(ss, cachedGeometry, clonedGeometry, ima[j], SPECULAR_MAP_UNIT , textureUnit)) ++textureUnit;
                    if (copyTextureCoordinateSet(ss, cachedGeometry, clonedGeometry, ima[j], REFLECTIVE_MAP_UNIT , textureUnit)) ++textureUnit;
