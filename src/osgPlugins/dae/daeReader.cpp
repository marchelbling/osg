...
void getSamplerSource(daeDatabase* database, std::string &samplerId, std::string &imageId) 
{
    ...
    {
        database->getElement((daeElement**)&param, imageIdx,   NULL, "source", NULL);
    ...
    {
        database->getElement((daeElement**)&param, imageIdx, NULL, "profile_COMMON", NULL);
        for ( unsigned int imageChildIdx = 0; imageChildIdx < param->getChildren().getCount(); ++imageChildIdx )
        {
...
unsigned int daeReader::transparencyMetaData(const std::string &renderEquation, unsigned int unit, 
                                                osg::StateSet *ss,  domCommon_transparent_type *ctt, 
                                                domCommon_float_or_param_type *pTransparency)
{
    ...
    if (pTransparency)
    {
        ...
        std::stringstream ssVal, ssName;
        ssVal << transparency;
        ssName << renderEquation.c_str() << "_transparency_amount";
        ss->setUserValue(ssName.str(), ssVal.str());
    }
  
    osg::Texture2D* pTransparentTexture = NULL;
    osg::Vec4 transparentColor(transparency, transparency, transparency, transparency);

    // defaults according to "Determining Transparency" chapter of 1.4.1 spec
    ...
    if (ctt)
    {
        ...
        if (ctt->getColor())
        {
            ...
            std::stringstream ssVal, ssName;
            ssVal << transparency;
            ssName << transparentColor[0] << "," << transparentColor[1] << "," << transparentColor[2] << "," << transparentColor[3] << " " << renderEquation.c_str() << "_transparency_color";
            ss->setUserValue(ssName.str(), ssVal.str());
        }
        else if (ctt->getTexture())
        {
            ...
            {
                unsigned int texCoordUnit = (_pluginOptions.usePredefinedTextureUnits ? TRANSPARENCY_MAP_UNIT : unit++);
                ss->setTextureAttributeAndModes(texCoordUnit, pTransparentTexture);			
                _texCoordSetMap[TextureToCoordSetMap::key_type(ss, TRANSPARENCY_MAP_UNIT)] = texCoordUnit;    
                { 
                    std::stringstream ssName, ssVal;
                    ssVal << texCoordUnit;
                    ssName << renderEquation.c_str() << "_transparency_" << "unit";
                    ss->setUserValue(ssName.str(), ssVal.str());
                }
                // detect Alpha masks cases      
                // texture has to be transformed to a GL_ALPHA format 
                // otherwise that won't work.          
                bool alpha_mask = true;
                osg::Image *srcImg =  pTransparentTexture->getImage();
                const int s = srcImg->s();
                const int t = srcImg->t();
                for (int i = 0; i < t; ++i)
                {
                    for (int j = 0; j < s; ++j)
                    {
                ...
                {                
                    std::stringstream ssName, ssVal;
                    ssName << renderEquation.c_str() << "_transparency_" << "alphamask";
                    ssVal << "1";
                    ss->setUserValue(ssName.str(), ssVal.str());
                }
            }
...
osg::Texture2D *daeReader::getTextureNotColladed(daeDatabase* database, std::string &samplerId) 
{
    ...
    database->getElement((daeElement**)&images, 0, NULL, COLLADA_TYPE_LIBRARY_IMAGES, NULL);
...
void daeReader::saveMaterialToStateSetMetaData(domMaterial*const material, osg::StateSet* stateset) 
{
    ...
    daeDatabase* database = _dae->getDatabase();
    //transparency  0 mean 100% opacity, changed in later collada versions (1 mean 100% opacity)...
    if (database) _invertTransparency = findInvertTransparency(database);// get which versions here
    ...
    if (database)
    {
        ...
        {
            database->getElement((daeElement**)&extra, el_number, NULL, COLLADA_TYPE_EXTRA, NULL);
                ...
                //if ( strcmp( teq->getProfile(), "custom" ) == 0 )
                {
                    ...
                    for( unsigned int currChild = 0; currChild < numChildren; ++currChild )
                    {
                        if ( teq->getContents()[currChild]->getElementType() == COLLADA_TYPE::ANY )
                        {
                            ...
                            else if ( strcmp( child->getElementName(), "shininess" ) == 0 )
                            {
                                //osg::notify(osg::NOTICE) <<  "IBL: " << child->getElementName() <<  std::endl;
                                unsigned int numChildren = child->getContents().getCount();
                                for ( unsigned int currChildBump = 0; currChildBump < numChildren; ++currChildBump )
                                {
                                    if ( strcmp( child->getContents()[currChildBump]->getElementName(), "texture" ) == 0 )
                                    {
                                        ...
                                        */
                                        ...
                                        std::string texPath = childTexture->getAttribute( "texture" );
                                        daeString texName = daeString(texPath.c_str());
                                        osg::Texture2D* tex =  getTexture(texName, SHININESS_MAP_UNIT);
                                        ...
                                        if (tex && tex->getImage())
                                        {
                                            unsigned int texCoordUnit = usePredefTexUnit ? SHININESS_MAP_UNIT : unit++;
                                            stateset->setTextureAttributeAndModes(texCoordUnit, tex);											
                                            std::string texCoord = childTexture->getAttribute( "texcoord" );
                                            _texCoordSetMap[TextureToCoordSetMap::key_type(stateset, SHININESS_MAP_UNIT)] = texCoordUnit;     

                                            int texUnit = getTexUnit(stateset, tex->getImage()->getFileName());
                                            ss << (texUnit != -1 ? texUnit : texCoordUnit);
                                            stateset->setUserValue("extra_shininess_unit", ss.str());
                                            ss.str("");
                                            ss <<  texCoord;
                                            stateset->setUserValue("extra_shininess_texcoord", ss.str());
                                            ss.str("");
                                            ...
                                            {
                                                std::string texValue = childTexture->getChild("extra")->getChild("technique")->getChild("amount")->getCharData();
                                                stateset->setUserValue("extra_shininess_amount", texValue);
                                            }
                                        }
                                    }
                                }
                            ...
                            //{
                                ...
                                //printDomAny( child );
                                //osg::notify(osg::NOTICE) << "Error: unsupported extra technique " << child->getElementName() << std::endl;
                            //}
                        }
